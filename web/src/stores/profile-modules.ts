 ...
 *  - POST /api/profile/dog-food
 *  - GET  /api/solar-terms
 *  - POST /api/solar-terms/claim
...
export const useProfileModulesStore = defineStore('profile-modules', () => {
  ...
  async function fetchSolarTerms(accountId: string) {
    ...
    try {
      ...
      throw new Error(e?.response?.data?.error || e?.message || '加载节气礼包失败')
    }
  }
  ...
  async function claimSolarTerms(accountId: string, solarTermId?: number) {
    ...
    try {
      const res = await api.post('/api/solar-terms/claim',
        { solarTermId },
        { headers: { 'x-account-id': accountId } },
      )
      if (!res.data?.ok)
  ...
  async function addDogFood(accountId: string, foodId: number, count = 1) {
    ...
    actionLoading.value = true
    try {
      const res = await api.post('/api/profile/dog-food',
        { foodId, count },
        { headers: { 'x-account-id': accountId } },
      )
      if (!res.data?.ok)
  ...
  async function fetchActivities(accountId: string) {
    ...
    try {
      ...
      throw new Error(e?.response?.data?.error || e?.message || '加载活动模块失败')
    }
  }
