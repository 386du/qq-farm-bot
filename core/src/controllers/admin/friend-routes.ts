...
function mountFriendRoutes(app: Application, ctx: AdminContext): void {
    ...
    // 获取"游戏内已拉黑"名单（含昵称/头像）
    app.get('/api/friend-blocked-gids', async (req: Request, res: Response) => {
        const id = getAccId(ctx, req);
        if (!id) return res.status(400).json({ ok: false, error: 'Missing x-account-id' });
        if (!checkAccountAccess(ctx, req as any, id)) {
            return res.status(403).json({ ok: false, error: '无权访问此账号' });
        }
        const gids = store.getFriendBlockedGids ? store.getFriendBlockedGids(id) : [];
        // 附加昵称/头像
        let friendsList: any[] = [];
        try {
        ...
        for (const f of friendsList) {
            const fgid = Number(f?.gid);
            if (fgid) nameMap.set(fgid, f);
        }
        const data = gids.map((gid: number) => {
            const f: any = nameMap.get(gid) || {};
            return {
                gid,
                name: String(f.name || f.nick || `GID:${gid}`).trim() || `GID:${gid}`,
    ...
    // 手动从"已拉黑名单"移除（不调用游戏 API；如需真正解除请到游戏内操作）
    app.post('/api/friend-blocked-gids/remove', async (req: Request, res: Response) => {
        const id = getAccId(ctx, req);
        if (!id) return res.status(400).json({ ok: false, error: 'Missing x-account-id' });
        if (!checkAccountAccess(ctx, req as any, id)) {
            return res.status(403).json({ ok: false, error: '无权访问此账号' });
        ...
        if (!gid) return res.status(400).json({ ok: false, error: 'Missing gid' });
        let ok = false;
        if (store.removeFriendBlockedGid) ok = !!store.removeFriendBlockedGid(id, gid);
        if (ctx.provider && typeof ctx.provider.broadcastConfig === 'function') {
            ctx.provider.broadcastConfig(id);
        }
        res.json({ ok, data: store.getFriendBlockedGids ? store.getFriendBlockedGids(id) : [] });
    });

    // 清空"已拉黑名单"
    app.post('/api/friend-blocked-gids/clear', async (req: Request, res: Response) => {
        const id = getAccId(ctx, req);
        if (!id) return res.status(400).json({ ok: false, error: 'Missing x-account-id' });
        if (!checkAccountAccess(ctx, req as any, id)) {
            return res.status(403).json({ ok: false, error: '无权访问此账号' });
        }
        if (store.setFriendBlockedGids) store.setFriendBlockedGids(id, []);
        if (ctx.provider && typeof ctx.provider.broadcastConfig === 'function') {
            ctx.provider.broadcastConfig(id);
        }
        res.json({ ok: true, data: [] });
